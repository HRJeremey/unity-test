...
public static class MeshUtils
{
    ...
    public static void AddToMeshArrays(Vector3[] vertices, Vector2[] uvs, int[] triangles, int index, Vector3 pos, float rot, Vector3 baseSize, Vector2 uv00, Vector2 uv11)
    {
        ...
        if (skewed)
        {
            vertices[vIndex0] = pos + GetQuaternionEuler(rot) * new Vector3(-baseSize.x, baseSize.z);
            vertices[vIndex1] = pos + GetQuaternionEuler(rot) * new Vector3(-baseSize.x, -baseSize.z);
            vertices[vIndex2] = pos + GetQuaternionEuler(rot) * new Vector3(baseSize.x, -baseSize.z);
            vertices[vIndex3] = pos + GetQuaternionEuler(rot) * baseSize;
        }
        else if (true)
        {

            vertices[vIndex0] = new Vector3(pos.x, 0.01f, pos.z) + new Vector3(0.5f, 0, 0.5f); // top right
            vertices[vIndex1] = new Vector3(pos.x, 0.01f, pos.z) + new Vector3(-0.5f, 0, 0.5f); // top left
            vertices[vIndex2] = new Vector3(pos.x, 0.01f, pos.z) + new Vector3(-0.5f, 0, -0.5f); // bottom right
            vertices[vIndex3] = new Vector3(pos.x, 0.01f, pos.z) + new Vector3(0.5f, 0, 0.5f); // bottom left
            Debug.Log($"vertices : {vertices[vIndex0]}, {vertices[vIndex1]}, {vertices[vIndex2]}, {vertices[vIndex3]}");
        }
        else
        {
            vertices[vIndex0] = pos + GetQuaternionEuler(rot - 270) * baseSize;
        ...
        //Relocate UVs
        uvs[vIndex0] = new Vector3(uv00.x, 0.01f, uv11.y);
        uvs[vIndex1] = new Vector3(uv00.x, 0.01f, uv00.y);
        uvs[vIndex2] = new Vector3(uv11.x, 0.01f, uv00.y);
        uvs[vIndex3] = new Vector3(uv11.x, 0.01f, uv11.y);
